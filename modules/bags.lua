pfUI:RegisterModule("bags", function ()
  ...
  function pfUI.bag:CreateAdditions(frame)
    ...
    if not frame.close then
      ...
      frame.close:SetTextColor(1,.25,.25,1)
      frame.close:SetFont("Interface\\AddOns\\pfUI\\fonts\\arial.ttf", pfUI_config.global.font_size, "OUTLINE")
    ...
    if not frame.bags then
      ...
      frame.bags.texture = frame.bags:CreateTexture(nil,"BACKGROUND")
      frame.bags.texture:SetTexture("Interface\\AddOns\\pfUI\\img\\arrow_up")
      frame.bags.texture:ClearAllPoints()
      frame.bags.texture:SetPoint("TOPLEFT", frame.bags, "TOPLEFT", 4, -2)
      frame.bags.texture:SetPoint("BOTTOMRIGHT", frame.bags, "BOTTOMRIGHT", -4, 2)
