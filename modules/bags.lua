pfUI:RegisterModule("bags", function ()
  ...
  pfUI.bag.button_size = (pfUI.bag:GetWidth() - pfUI_config.bars.border*3) / 10 - pfUI_config.bars.border
  pfUI.bag:Hide()
  ...
  function pfUI.bag:UpdateBag(arg1)
    if not pfUI.bag.initialized then
      pfUI.bag.initialized = "yes"
      pfUI.bag:SetupPositions()
      return
    end

    for i=1, GetContainerNumSlots(arg1) do
      local f = getglobal("ContainerFrame" .. arg1 + 1 .. "Item" .. i)
      if f then
        local texture, itemCount, locked, quality, readable = GetContainerItemInfo(arg1, i);
        ...
        SetItemButtonCount(getglobal("ContainerFrame" .. arg1 + 1 .. "Item" .. i), itemCount);
        if ( texture ) then
          local cooldown = getglobal(getglobal("ContainerFrame" .. arg1 + 1 .. "Item" .. i):GetName().."Cooldown");
          local start, duration, enable = GetContainerItemCooldown(arg1, getglobal("ContainerFrame" .. arg1 + 1 .. "Item" .. i):GetID());
          CooldownFrame_SetTimer(cooldown, start, duration, enable);
          if ( duration > 0 and enable == 0 ) then
            SetItemButtonTextureVertexColor(button, 0.4, 0.4, 0.4);
          end
          getglobal("ContainerFrame" .. arg1 + 1 .. "Item" .. i .. "Cooldown"):Show();
        else
          getglobal("ContainerFrame" .. arg1 + 1 .. "Item" .. i .. "Cooldown"):Hide();
        end
      end
    end
  end

  function pfUI.bag:SetupPositions()
    -- setup bag positions
    local x = -1
    local y = 0
    local x_max = 10
    ...
    for j=1, 5 do
      for i=1, GetContainerNumSlots(j-1) do
        ...
        local f = getglobal("ContainerFrame" .. j .. "Item" .. i)
        f:SetParent(pfUI.bag[j])
        f:SetID(i)
        f:SetPoint("TOPLEFT", pfUI.bag, "TOPLEFT", pfUI_config.bars.border*2 + x*pfUI.bag.button_size + x*pfUI_config.bars.border, -pfUI_config.bars.border*2 - y*pfUI.bag.button_size - y * pfUI_config.bars.border - 17)
        f:Show()
        f:SetHeight(pfUI.bag.button_size)
        f:SetWidth(pfUI.bag.button_size)
    ...
    y = y + 1
    pfUI.bag:SetHeight( y * pfUI.bag.button_size + y * pfUI_config.bars.border + pfUI_config.bars.border*3 + 17 + 17)

    -- setup bag overview panel
    pfUI.bag.bagframe:SetPoint("BOTTOMLEFT", pfUI.bag, "TOPLEFT", 0, pfUI_config.bars.border)
    pfUI.bag.bagframe:SetWidth(3 * pfUI_config.bars.border + 4*(pfUI_config.bars.border+pfUI.bag.button_size))
    pfUI.bag.bagframe:SetHeight(pfUI.bag.button_size + 2 * (pfUI_config.bars.border*2))

    for i=0, 3 do
      local bag = getglobal("CharacterBag" .. i .. "Slot")
      bag:ClearAllPoints()
      bag:SetParent(pfUI.bag.bagframe)
      bag:SetWidth(pfUI.bag.button_size)
      bag:SetHeight(pfUI.bag.button_size)
      ...
      bag:SetPoint("TOPLEFT", pfUI.bag.bagframe, "TOPLEFT", pfUI_config.bars.border*2 + i*pfUI.bag.button_size + i*pfUI_config.bars.border, -pfUI_config.bars.border*2)
      bag:SetAlpha(1)
      bag:Show()
    end
  end

  -- close pfUI.bag on escape
  tinsert(UISpecialFrames,"pfBag");

  pfUI.bag:SetScript("OnShow", function ()
        pfUI.bag:SetupPositions()
  end)

  -- bag events (all necessary?)
  ...
  pfUI.bank:SetScript("OnEvent", function()
    ...
    if not pfUI.bank:IsShown() and pfUI.bank.initialized then
      message("ignore")
      pfUI.bank.initialized = "yes"
      return
    end
