    ...
    cooldown.cd:SetScript("OnUpdate", function()
      ...
      if remaining >= 0 then
        ...
        if remaining > 99 then
          ...
          unit = "m"
          r,g,b,a = pfUI:strsplit(",", pfUI_config.appearance.cd.mincolor)
        end
        if remaining > 99 then
          remaining = remaining / 60
          unit = "h"
          r,g,b,a = pfUI:strsplit(",", pfUI_config.appearance.cd.hourcolor)
        end
        if remaining > 99 then
          remaining = remaining / 24
          unit = "d"
          r,g,b,a = pfUI:strsplit(",", pfUI_config.appearance.cd.daycolor)
        end
        this.text:SetText(pfUI:round(remaining) .. unit)
