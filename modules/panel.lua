pfUI:RegisterModule("panel", function()
        ...
        for i=1, repairToolTip:NumLines() do
          ...
          if tmpText and tmpText:GetText() then
            ...
            if lval and rval and lval ~= rval then
              ...
              table.insert(itemRepArray[itemLink], repCost)
              break
            end
          end
        end
  ...
  function pfUI.panel:UpdateRepair()
	...
	for _, v in pairs(GetDurabilityState()) do
      local percent = v[1]
    ...
    local tooltip = function()
      -- show tooltip
      if not next(GetDurabilityState()) then return end
	  
	  local totalCost = 0
      GameTooltip:ClearLines()
      GameTooltip_SetDefaultAnchor(GameTooltip, this)
	  for itemName,v in pairs(GetDurabilityState()) do
	    local percent, curCost = v[1], v[2]
	    local r,g,b
		if percent > 50 then
			r, g, b = .49, .99, 0
		elseif percent < 50 and percent > 10 then
			r, g, b = 1, .56, 0
        ...
        GameTooltip:AddDoubleLine(itemName, percent.." %", _, _, _, r, g, b)
