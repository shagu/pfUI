pfUI:RegisterModule("panel", function()
  ...
  function pfUI.panel:UpdateRepair()
	...
	local curItemPercent
	local totalCost, summPercent, repItemCount = 0, 0, 0

    for i,slotName in pairs(slotnames) do
      local id = GetInventorySlotInfo(slotName.. "Slot")
      repairToolTip:Hide()
      repairToolTip:SetOwner(this, "ANCHOR_LEFT")
      local hasItem, _, curItemCost = repairToolTip:SetInventoryItem("player", id)
      totalCost = totalCost + curItemCost -- calculate repair costs
      if not hasItem then
        ...
        for i=1, repairToolTip:NumLines() do
          ...
          if tmpText and tmpText:GetText() then
            ...
            if lval and rval then
              curItemPercent = math.ceil(lval / rval * 100)
			  repItemCount = repItemCount + 1
              break
            end
          end
        end
        summPercent = summPercent + curItemPercent
      end
    end
    local repPercent = math.ceil(summPercent/repItemCount)
    repairToolTip:Hide()
...
local tooltip = function()
      ...
      SetTooltipMoney(GameTooltip, totalCost)
      GameTooltip:Show()
    end
    ...
    pfUI.panel:OutputPanel("durability", ARMOR .. ": " .. repPercent .. "%", tooltip, click)
  end
